...
class FitsDownloader:
    ...
    async def download_single_fits(
        self, obsid: int, session: aiohttp.ClientSession
    ) -> None:
        for retry in range(self.max_retrys):
            try:
                async with self.sem:
                    async with session.get(
                        self.url, params={"obsid": obsid, "token": self.TOKEN}
                    ) as response:
                        fits_name = response.headers["Content-Disposition"].split("=")[
                            1
                        ]
                        ...
                        print(
                            f"<{fits_name} has dowloaded,current progress:{self.task_completed}/{self.task_total}>"
                        )
                        return
                ...
                if retry == self.max_retrys - 1:
                    raise aiohttp.http_exceptions.HttpProcessingError(
                        code=500, message="Download failed"
                    ) from e

    @asyncio_decorator
    async def async_download_fits(self, obsids_list: list[str]):
        tasks = []
        async with aiohttp.ClientSession() as session:
            for obsid in obsids_list:
                obsid = str(obsid)
                task = self.download_single_fits(obsid=obsid, session=session)
                tasks.append(task)
...
def download_fits(
    ...
    fits_downloader = FitsDownloader(
